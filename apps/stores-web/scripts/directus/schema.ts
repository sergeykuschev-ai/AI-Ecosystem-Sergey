...
export const schema: CollectionSpec[] = [
  {
    ...
    fields: [
      ...
      textField("description", { sort: 7, meta: { translations: ru("Описание 1С (не используется на сайте)") } }),
      stringField("site_name", {
        sort: 8, meta: { translations: ru("Название на сайте") },
        schema: { data_type: "varchar", length: 512, nullable: true },
      }),
      textField("site_description", { sort: 9, meta: { translations: ru("Описание на сайте") } }),
      fileField("site_image", { sort: 10, meta: { translations: ru("Изображение на сайте") } }),
      dropdownField("content_status", [
        { text: "Только данные 1С", value: "source_only" },
        { text: "Обогащается", value: "enriching" },
        { text: "Нужна проверка", value: "review" },
        { text: "Готово", value: "ready" },
      ], { sort: 11, schema: { data_type: "varchar", length: 32, nullable: true } }),
      stringField("site_section", { sort: 12, meta: { translations: ru("Раздел сайта") } }),
      stringField("site_category", { sort: 13, meta: { translations: ru("Категория сайта") } }),
      stringField("site_subcategory", { sort: 14, meta: { translations: ru("Подкатегория сайта") } }),
      stringField("brand", { sort: 15, meta: { translations: ru("Бренд") } }),
      dropdownField("classification_status", [
        { text: "Автоматически", value: "auto" },
        { text: "Нужна проверка", value: "review" },
        { text: "Проверено вручную", value: "manual" },
      ], { sort: 16, schema: { data_type: "varchar", length: 32, nullable: true } }),
      integerField("classification_confidence", { sort: 17, meta: hiddenSystem }),
      textField("classification_reason", { sort: 18, meta: hiddenSystem }),
      timestampField("classification_synced_at", { sort: 19, meta: hiddenSystem }),
      stringField("content_source_url", {
        sort: 20, meta: { translations: ru("Источник контента") },
        schema: { data_type: "varchar", length: 2048, nullable: true },
      }),
      stringField("image_source_url", {
        sort: 21, meta: { translations: ru("Источник изображения") },
        schema: { data_type: "varchar", length: 2048, nullable: true },
      }),
      stringField("source_title", {
        sort: 22, meta: hiddenSystem, schema: { data_type: "varchar", length: 1024, nullable: true },
      }),
      textField("source_description", { sort: 23, meta: hiddenSystem }),
      timestampField("content_synced_at", { sort: 24, meta: hiddenSystem }),
      decimalField("price", 12, 2, { sort: 25, meta: { translations: ru("Цена Миска") } }),
      decimalField("stock_quantity", 14, 3, { sort: 26, meta: { translations: ru("Остаток Миска") } }),
      decimalField("stock_quantity_raw", 14, 3, { sort: 27, meta: hiddenSystem }),
      timestampField("offers_synced_at", { sort: 28, meta: hiddenSystem }),
      booleanField("active", {
        sort: 29,
        meta: { translations: ru("Активно"), required: true },
        schema: { data_type: "boolean", nullable: false, default: false },
      }),
      timestampField("synced_at", { sort: 30, meta: hiddenSystem }),
    ],
  },
  {
