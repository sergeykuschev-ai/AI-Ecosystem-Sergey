const SOURCE_URL = "https://ekoprom.org/brands/ekoprom/happy-jungle/";
...
function absolute(value) {
  if (!value) return "";
  return new URL(value.replace("/200x/", "/"), SOURCE_URL).href;
}

export function parseHappyJungleBrandPage(html) {
  ...
  for (const block of blocks) {
    ...
    const composition = properties.match(/Состав:\s*([^.]+(?:\.[^К]*?)?)(?=Кол-во|Штрихкод|$)/i)?.[1]?.trim() ?? "";
    if (!sku || !title || !image) continue;
    products.push({
      sku,
      barcode,
      title,
      description: [intro, composition ? "Состав: " + composition.replace(/[.;\s]+$/g, "") + "." : ""].filter(Boolean).join(" "),
      imageUrl: absolute(image),
      sourceUrl: new URL(href, SOURCE_URL).href,
    });
  }
  return products;
}
...
export async function fetchHappyJungleCatalog() {
  const products = parseHappyJungleBrandPage(await fetchText(SOURCE_URL));
  const bySku = new Map();
  ...
  return { products, bySku, byBarcode, sourceUrl: SOURCE_URL };
}
