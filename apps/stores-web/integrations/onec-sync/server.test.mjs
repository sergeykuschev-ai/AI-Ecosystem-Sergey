...
async function start(enabled = true) {
  storageDir = await mkdtemp(join(tmpdir(), "onec-sync-"));
  process.env.ONEC_EXCHANGE_ENABLED = String(enabled);
  process.env.ONEC_EXCHANGE_USERNAME = "miska-test";
...
function basic(username = "miska-test", password = "test-password") {
  return `Basic ${Buffer.from(`${username}:${password}`).toString("base64")}`;
}

test("disabled receiver does not expose the exchange", async () => {
  const base = await start(false);
  const response = await fetch(`${base}/api/1c/exchange?type=catalog&mode=checkauth`);
  assert.equal(response.status, 404);
  assert.match(await response.text(), /^failure/);
});

test("CommerceML checkauth, init, chunk upload and staging work end to end", async () => {
  ...
  const uploadUrl = `${base}/api/1c/exchange?type=catalog&mode=file&filename=import.xml`;
