...
async function targets(c){const q=new URLSearchParams({fields:"id,external_id,sku,barcode,name,site_name,site_description,site_image,site_category,content_status","filter[brand][_eq]":"Мнямс","filter[stock_quantity][_gt]":"0",limit:"-1"});return (await req(c,"GET","/items/miska_catalog_products?"+q))?.data??[]}
export async function enrichMnyams({dryRun=false}={}){
 ...
 const ts=new Date().toISOString();for(const {p,s} of matched){const name=p.site_name||s.title;const desc=p.site_description||composePetProductDescription({title:name,sourceDescription:s.description,siteCategory:p.site_category});await req(c,"PATCH","/items/miska_catalog_products/"+p.id,{site_name:name,site_description:desc,content_source_url:s.sourceUrl,image_source_url:s.imageUrl,source_title:s.title,source_description:s.description,content_status:desc&&p.site_image?"ready":"enriching",content_synced_at:ts})}return{targets:todo.length,matched:matched.length,unmatched,failures:official.failures,updated:matched.length}
}
