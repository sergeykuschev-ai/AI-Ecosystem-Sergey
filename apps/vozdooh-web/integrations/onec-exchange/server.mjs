...
const COOKIE_NAME = "vozdooh_1c_session";
const DEFAULT_FILE_LIMIT = 5 * 1024 * 1024;
const DEFAULT_MAX_FILE_SIZE = 256 * 1024 * 1024;
const SESSION_TTL_MS = 60 * 60 * 1000;

const env = (name) => process.env[name]?.trim() ?? "";
const enabled = () => env("ONEC_EXCHANGE_ENABLED").toLowerCase() === "true";
...
function authenticated(req) {
  ...
  const password = env("ONEC_EXCHANGE_PASSWORD");
  return Boolean(credentials && username && password &&
...
function sessionRoot(token) {
  const root = env("ONEC_EXCHANGE_STORAGE_DIR") || "/var/lib/vozdooh-onec-exchange";
...
function logEvent(mode, filename, result) {
  ...
  console.info("VOZDOOH 1C exchange", { mode: safeMode, filename: safeFile, result });
}
...
export async function handleRequest(req, res) {
  ...
  let url;
  try { url = new URL(req.url ?? "/", "http://vozdooh-onec.local"); }
  catch { fail(res, "invalid request URL"); return; }

  if (url.pathname !== "/api/1c/exchange") { fail(res, "not found", 404); return; }
  if (!enabled()) { fail(res, "exchange disabled", 404); return; }
  ...
  if (!token) { fail(res, "session is missing or expired", 401); return; }
  ...
  if (mode === "file") {
    if (req.method !== "POST") { fail(res, "file mode requires POST", 405); return; }
    try {
      ...
      fail(res, message);
      console.warn("VOZDOOH 1C file rejected", { mode, reason: message });
    }
    return;
  }
  ...
  if (mode === "import") {
    try {
      ...
      fail(res, message);
      console.warn("VOZDOOH 1C import rejected", { mode, reason: message });
    }
    return;
  }
...
export function createExchangeServer() {
  return createServer((req, res) => {
    handleRequest(req, res).catch((error) => {
      ...
      console.error("VOZDOOH 1C unhandled error", { reason: message });
...
if (executedDirectly) {
  ...
  server.listen(port, "0.0.0.0", () => {
    console.info(`VOZDOOH 1C exchange receiver listening on port ${port}`);
  });
}
